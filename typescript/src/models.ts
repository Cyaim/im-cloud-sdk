/**
 * Payload and request types for tiers T0–T3 and for `desk.*` from T4, generated by hand from
 * `sdk/endpoint-inventory.json` (`payloadTypes`) and checked against the controllers it was
 * generated from.
 *
 * `payloadTypes` records the **C# declaration, not the wire form**, and the difference is not
 * cosmetic for T3: every message id it lists as `long` (`MessageReceipt`, `PinnedMessage`,
 * `MessageBrief`, `Message`) is written with `WriteAsString` and arrives as a JSON string, and the
 * T3 request DTOs declare their message ids as C# `string` outright. So the rule below holds on
 * both sides of every T3 call.
 * payloadTypes 记录的是 C# 声明而不是线上形态：它标成 long 的消息 id 在线上都是字符串，
 * 而 T3 的请求 DTO 干脆把消息 id 声明成 string。
 *
...
/**
 * One message inside one conversation — the body of `msg.pin`, `msg.unpin`, `msg.favourite`,
 * `msg.unfavourite` and `msg.burn`. The pair is the only way to address a message: ids are unique
 * per conversation, not per tenant.
 *
 * **`messageId` must leave as a string, and here that is the server's own requirement rather than
 * this SDK's caution.** The DTO declares it C# `string`, and the socket binder refuses a JSON number
 * for a string property outright — the call comes back `1000 InternalError`, not a 1001 that names
 * the field. An id the server cannot parse is read as 0, which four of the five endpoints answer
 * without complaint (see each method for which).
 * messageId 必须是字符串：服务端 DTO 声明的就是 string，套接字绑定器遇到 JSON 数字直接抛，回的是 1000。
 */
...
/**
 * Cursor paging over one of the caller's own lists (`msg.favourites`).
 *
 * The body may be omitted. **The page size is the extras module's, not the socket's:** absent or 0
 * means 20, anything above 100 is cut to 100 — both deployment options (`IM:Extras`), so treat the
 * numbers as defaults rather than constants. That differs from {@link CursorRequest}, where absent
 * means 50.
 */
export interface PageRequest {
  /** Opaque, from the previous page's `nextCursor`. A malformed one restarts at page one. */
  cursor?: string | null;
  /** Absent or 0 → 20; capped at 100. */
  limit?: number;
}
 ...
 * Body of `msg.search`.
 *
 * **`keyword` is required even though the server's C# type says nullable**: absent or blank is
 * `1001`. Every other field narrows, and the four filters (`contentTypes`, `senderId`,
 * `startTime`, `endTime`) are applied **after** the index returns a page — so a short or empty page
 * with `hasMore: true` is normal. Page on `nextCursor`, never on `items.length`.
 */
export interface SearchMessagesRequest {
   ...
   * it. A conversation the caller cannot see is `1103` — group non-members included, not `1503`.
   */
  conversationId?: string;
  /** Integers on the wire, like every enum here. */
  ...
  endTime?: number;
  cursor?: string | null;
  /** Absent, 0 or negative → 20; capped at 100. */
  limit?: number;
}

/**
 * Body of `msg.receiptDetail`. `messageId` is a C# `string` on the server, so it must travel as a
 * JSON string — see {@link ConversationMessageRequest}.
 */
...
/**
 * Who has read one message, as `msg.receiptDetail` returns it.
 *
 * **`totalCount` includes the sender; `readUserIds` never does.** So "read by everyone" is
 * `readCount === totalCount - 1`, not `readCount === totalCount` — 2 in a single chat means "the
 * other person", and a UI comparing the two numbers directly never shows a message as fully read.
 * `totalCount` is a snapshot taken at the last receipt, so members who joined later are not in it.
...
export interface MessageReceipt {
  ...
  messageId: string;
  /** Always present, `[]` when nobody has read it. Never contains the sender. */
  readUserIds: string[];
  readCount: number;
  /** Everyone expected to read it, **sender included**. */
  totalCount: number;
  /** Unix ms. Equals the message's `createTime` on an empty receipt. */
  updatedAt: number;
}
 ...
 *
 * `brief` is built at read time, so it reflects the message as it is now: a recalled message reads
 * `digest: "[Recalled]"` with `recalled: true`. Digest tokens such as `"[Image]"` are fixed wire
 * values meant to be localised by the client, not shown as they are.
 */
export interface PinnedMessage {
  ...
  /** Unix ms. */
  pinnedAt: number;
  /** Always filled by `msg.pins`; optional because the server type allows null. */
  brief?: MessageBrief | null;
}
...
/** Body of `conv.markUnread` (T3). */
export interface MarkUnreadRequest {
  conversationId: string;
  /**
   * **Absent means `true`** — a C# property initialiser, not a nullable — so clearing the mark is
   * `unread: false`, sent explicitly. `conv.read`, `conv.delete` and `conv.clear` clear it too.
   * 省略即 true：清除标记必须显式发 false。
   */
  unread?: boolean;
}
...
/** Body of `user.setStatus` (T3): the caller's own free-text status line. */
export interface SetStatusRequest {
  /**
   * Trimmed; more than 64 characters is `1001`. **Absent, null or blank clears it** — so
   * `setStatus({})` is how a status is removed.
   */
  status?: string;
}
...
/**
 * Body of `friend.setRemark` (T3) — what *you* call a contact, and which of your groups they are
 * filed in. Never shown to them.
 *
 * **The two optional fields are not symmetric, and the asymmetry is the server's.** An absent
 * `remark` *clears* the remark; an absent `tags` leaves the tags alone. So changing only the tags
 * means resending the current remark with them, or the remark is lost in the same call.
 * 两个可选字段不对称：省略 remark 是清空备注，省略 tags 是不动标签。只改标签时要把现有备注一起带上。
 */
export interface SetRemarkRequest {
  /** The contact. Not a contact is `1302 NotFriend`. */
  userId: string;
  /** At most 64 characters — refused `1001` beyond that, never truncated. Absent, null or blank **clears** it. */
  remark?: string;
  /** Absent leaves the tags unchanged; `[]` clears them. At most 20, each 1–32 characters, else `1001`. */
  tags?: string[];
}
...
/**
 * A cursor over one group's list — `group.memberList`, and `group.applicationList`, where an empty
 * `groupId` means "every group I manage".
 */
export interface GroupCursorRequest {
  /** Required by `group.memberList`. For `group.applicationList`, `''` lists across every group the caller manages. */
  groupId: string;
  cursor?: string | null;
  /** 1…200 is kept; anything else — absent, 0, or over 200 — becomes 50. A super group is never listed in one frame. */
  limit?: number;
}
...
/**
 * A request to join a group that needs approval, or an ordinary member's invitation into one — as
 * `group.applicationList` returns it.
 *
 * **Every status comes back, not only pending ones**: the server's list has no status filter. A
 * "requests awaiting you" badge has to filter on `status === ApplicationStatus.Pending` itself, or
 * it counts every request ever handled. `Expired` is declared and never produced today.
 * 列表返回所有状态，不只是待处理的：服务端没有按状态过滤。角标要自己筛 Pending。
 */
export interface GroupApplication {
  appId: string;
  groupId: string;
  /** The user who wants in. */
  applicantId: string;
  /** Set when an ordinary member invited them into a group that needs approval. */
  inviterId?: string | null;
  reason?: string | null;
  status: ApplicationStatus;
  /** The owner or admin who handled it. */
  handlerId?: string | null;
  handleReason?: string | null;
  /** Unix ms. */
  createdAt: number;
  /** Unix ms. Absent while pending. */
  handledAt?: number | null;
}

/** Body of `group.transfer`. */
export interface TransferOwnerRequest {
  groupId: string;
  /** Must already be a member (`1503` otherwise), and must not be the caller (`1001`). */
  newOwnerId: string;
}

/** Body of `group.handleApplication`. */
export interface HandleApplicationRequest {
  groupId: string;
  applicantId: string;
  /**
   * **Required here although the server does not require it: absent means reject.** The C# default
   * is `false`, so a body that forgot the field would turn somebody away without anyone deciding
   * to. The type makes the decision explicit instead.
   * 服务端省略即拒绝，所以这里设成必填：忘了写的请求会在没人决定的情况下拒掉一个人。
   */
  accept: boolean;
  /** Stored as `handleReason`. */
  reason?: string;
}

/** Body of `group.setRole`. */
export interface SetRoleRequest {
  groupId: string;
  userId: string;
  /**
   * **`GroupRole.Member` or `GroupRole.Admin`, and nothing else — by type, because the server does
   * not check.** It refuses `Owner` (`1008`, "use group.transfer") but stores any other integer it
   * is given, and two of them are privilege bugs: `0` escapes a group-wide mute (only `Member` is
   * muted), and `4` or above counts as a manager that outranks every admin. The closed type is what
   * stops that being one typo away. Omitting the field server-side means `Member`, so the property
   * is required here too — a demotion should never be the result of leaving something out.
   * 只接受 Member / Admin：服务端拒 Owner，却会原样存下其它任何整数——0 能躲过全员禁言，4 以上比管理员还大。
   */
  role: typeof GroupRole.Member | typeof GroupRole.Admin;
}

/** Body of `group.mute` — the group-wide switch. */
export interface MuteGroupRequest {
  groupId: string;
  /** **Absent means `true`.** Unmuting is `mute: false`, sent explicitly; `untilMs` is then ignored. */
  mute?: boolean;
  /**
   * Unix ms the mute lasts until. Absent with `mute: true` is indefinite.
   *
   ...
   * deadline. A value computed as `Date.now() + duration` is safe; one taken from an old form state
   * is not.
   * 不要发已经过去的时间：服务端把它当成「无限期」，而不是「解除」。
   */
  untilMs?: number;
}

/** Body of `group.muteMember` — one member. */
export interface MuteMemberRequest {
  groupId: string;
  userId: string;
  /**
   * Unix ms the member stays muted until. **Absent, null or in the past unmutes**; there is no
   * indefinite member mute — send a far-future time for one. Note the opposite reading from
   * {@link MuteGroupRequest.untilMs}, where a past time mutes indefinitely.
   * 缺省或过去的时间即解除；没有「无限期禁言个人」，要就发一个很远的时间。与全员禁言的读法正好相反。
   */
  untilMs?: number;
}

/** Body of `group.setNickname` — the name somebody goes by inside one group. */
export interface SetGroupNicknameRequest {
  groupId: string;
  /**
   * Whose nickname. **Absent, `''` or the caller's own id means the caller**, which any member may
   * set. Anybody else's needs owner or admin *and* outranking them (`1504` / `1511`).
   */
  userId?: string;
  /** Trimmed; absent or blank clears it. Longer than 64 characters is **silently truncated**, not refused. */
  nickname?: string;
}

/** Body of `group.announcement`. */
export interface AnnouncementRequest {
  groupId: string;
  /**
   * Trimmed; absent, null or blank clears it. Longer than 4096 characters is **silently truncated**,
   * not refused — so read the group back if the exact text matters.
   */
  announcement?: string;
}
